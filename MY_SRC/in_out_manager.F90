...
!$AGRIF_END_DO_NOT_TREAT
   ...
   INTEGER ::   numflo          =   -1      !: logical unit for floats ascii output
      !
! necessary for BLING (xianmin/AGT)
#if defined key_top
   INTEGER ::   numphp     = -1         !: php.budget
   INTEGER ::   numfed     = -1         !: fed.budget
   INTEGER ::   numoxy     = -1         !: oxy.budget
   INTEGER ::   numsms     = -1         !: sms.budget
   INTEGER ::   numsms2    = -1         !: sms2.budget
   INTEGER ::   numphpext  = -1         !:
   INTEGER ::   numfedext  = -1
   INTEGER ::   numoxyext  = -1
   INTEGER ::   numcar     = -1         !: car.budget
#endif
