...
subroutine FMS_ocmip2_co2calc(isd,ied,jsd,jed,isc,iec,jsc,jec, mask,            &
                          ...
                          htotalhi, htotalslab, co2star, alpha, pCO2surf, co3_ion)  !{
...
!
        ...
        htotalslab(i,j) = drtsafe(  k1, k2, kb, k1p, k2p, k3p, ksi, kw,   &
                                ks, kf, bt, dic_in(i,j), ft, pt_in(i,j),&
                                sit_in(i,j), st, ta_in(i,j),            &
                                htotalhi(i,j), htotallo(i,j),htotalslab(i,j), xacc)
!
! Calculate [CO2*] as defined in DOE Methods Handbook 1994 Ver.2, 
! ORNL/CDIAC-74, Dickson and Goyet, eds. (Ch 2 p 10, Eq A.49)
!
        htotal2 = htotalslab(i,j) * htotalslab(i,j)
        co2star_internal = dic_in(i,j) * htotal2 / (htotal2 +           &
                         k1 * htotalslab(i,j) + k1 * k2)
